...
public class RemoteDeploymentAdminImpl implements Remote, RemoteDeploymentAdmin, EventHandler {
	...
	private boolean registerEventHandler() {
		if (eventHandleRegistration == null) {
			...
			eventProps.put(EventConstants.EVENT_TOPIC, new String[] { INSTALL_EVENT_TOPIC,
	...
	public Object uninstallDeploymentPackage(String dpName, String version, boolean force) {
		...
		if (force) {
			try {
				...
				Error error = new Error(org.tigris.mtoolkit.iagent.internal.utils.ExceptionCodeHelper.fromDeploymentExceptionCode(e.getCode()),
					"Failed to uninstall deployment package: " + e.getMessage());
	...
	public Object installDeploymentPackage(InputStream in) {
		...
		try {
			...
			Error error = new Error(org.tigris.mtoolkit.iagent.internal.utils.ExceptionCodeHelper.fromDeploymentExceptionCode(e.getCode()),
				"Failed to install deployment package: " + e.getMessage());
	...
	private void handleInstall(String symbolicName) {
		DebugUtils.log(this, "[handleInstall] newSymbolicName: "
						+ symbolicName
						+ "; oldSymbolicName: "
						+ dpSymbolicName);
	...
	private void handleUninstall(String symbolicName) {
		...
		if (dpSymbolicName != null) {
			DebugUtils.log(this, "[handleUninstall] Uninstallation already in progress for: "
							+ dpSymbolicName
				...
				if (successful) {
					...
					if (synchronizer != null) {
						if (dpVersion != null) { // uninstall event
							...
							dpVersion = dp.getVersion().toString();
							DebugUtils.log(this, "[handleComplete] Install event completed for: "
											+ dpSymbolicName
											+ "_"
											+ dpVersion);
