...
public class DataFormater {
	...
	/**
	 * Define IDs of basic simple types in message protocol.
	 */
	...
	public static final byte ID_STRING = 0x09;
	public static final byte ARRAY_FLAG = (byte) 0x80;

	/**
	 * Define size in bytes of basic simple types in message protocol.
	 */
	 ...
	 * @param data
	 *            the byte which is needed to be appended
	 * @throws IOException
	 *             in case of output stream write error
	 */
	 ...
	 * 
	 * @param os
	 *            stream to write data in
	 * @param value
	 *            the integer needed to be appended
	 * @throws IOException
	 ...
	 *            stream to write data in
	 * @param value
	 *            the short needed to be appended
	 * @throws IOException
	 *             in case of write error
	 */
	...
	public static long getLong(InputStream is) throws IOException {
		...
		if ((byte0 | byte1 | byte2 | byte3 | byte4 | byte5 | byte6 | byte7) < 0)
			throw new EOFException();
		return ((byte0 & 0xFFL) << 56) | ((byte1 & 0xFFL) << 48) | ((byte2 & 0xFFL) << 40) | ((byte3 & 0xFFL) << 32)
				| ((byte4 & 0xFFL) << 24) | ((byte5 & 0xFFL) << 16) | ((byte6 & 0xFFL) << 8) | (byte7 & 0xFFL);
	}
	 ...
	 *            stream to write data in
	 * @param value
	 *            long value needed to be appended
	 * @throws IOException
	 *             in case of write error
	 */
	...
	public static void writeChar(OutputStream os, byte value) throws IOException {
		appendByte(os, ID_C_CHAR);
		appendByte(os, value);
	}

	/**
	 * First reads ID_C_CHAR and then the value.
	 * 
	 * @param is
	 * @throws IOException
	 */
	public static byte readChar(InputStream is) throws IOException {
		if (getByte(is) != ID_C_CHAR) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString(ID_C_CHAR));
		}
		return getByte(is);
	}

	/**
	 * First writes ID_C_CHAR | ARRAY_FLAG array size and then the value.
	 * 
	 * @param os
	 * @param value
	 * @throws IOException
	...
	public static void writeCharArray(OutputStream os, byte[] value) throws IOException {
		appendByte(os, (byte) (ID_C_CHAR | ARRAY_FLAG));
		appendByteArray(os, value);
	}
	...
	public static byte[] readCharArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_CHAR | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_CHAR | ARRAY_FLAG) & 0xFF));
		}
		return getByteArray(is);
	}
	...
	public static short[] readShortArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_SHORT | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_SHORT | ARRAY_FLAG) & 0xFF));
		}
	...
	public static short[] readWordArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_WORD | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_WORD | ARRAY_FLAG) & 0xFF));
		}
		return getShortArray(is);
	}
	...
	public static int[] readIntArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_INT | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_INT | ARRAY_FLAG) & 0xFF));
		}
	...
	public static int[] readDWordArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_DWORD | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_DWORD | ARRAY_FLAG) & 0xFF));
		}
		return getIntArray(is);
	}
	...
	public static long[] readLongArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_LONG | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_LONG | ARRAY_FLAG) & 0xFF));
		}
	...
	public static long[] readQWordArray(InputStream is) throws IOException {
		if (getByte(is) != (byte) (ID_C_QWORD | ARRAY_FLAG)) {
			throw new IOException("Expects data type: 0x" + Integer.toHexString((ID_C_QWORD | ARRAY_FLAG) & 0xFF));
		}
		return getLongArray(is);
	}
	...
	/**
	 * String array is represend as ID_C_INT numbers of strings and following
	 * UTF8 byte arrays.
	 * 
	 * @param os
	 * @param value
	...
	/**
	 * First reads ID_C_INT which is number of strings followd and then UTF8
	 * byte arrays.
	 * 
	 * @param is
	 * @throws IOException
