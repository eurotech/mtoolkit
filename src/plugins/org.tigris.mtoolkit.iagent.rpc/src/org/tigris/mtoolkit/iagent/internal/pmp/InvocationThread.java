  ...
  public void run() {
    ...
    try {
      ...
      if (exc instanceof InvocationTargetException) {
        ...
        errMsg = exc.toString();
        DebugUtils.error(session, "Invocation Error", exc);
      }
    ...
    if (returnType.equals(PMPData.TYPES1[8]) || returnType.equals(PMPData.TYPES2[8])) {
      try {
        ...
        DebugUtils.error(session, PMPSessionThread.ERRMSG2, exc);
      }
      ...
      if (result instanceof InputStream && !(result instanceof Remote)) {
        ...
        interfaces = ((Remote) result).remoteInterfaces();
          ...
          || !PMPServiceImpl.checkInstance(interfaces = ((Remote) result).remoteInterfaces(), result.getClass())) {
        String errMsg = "Method result " + result + " Is not instance of " + Remote.class.getName()
