...
import org.tigris.mtoolkit.dpeditor.editor.DPPConstants;
 ...
 */
public class CustomHeadersDialog extends Dialog implements SelectionListener,
	...
	private static CustomHeadersDialog dialog;
	 ...
	 */
	public static CustomHeadersDialog getInstance(Shell parent, Point displayLoc, Point size) {
		if (dialog == null) {
			SHELL_WIDTH = 330;
			SHELL_HEIGHT = 450;
			dialog = new CustomHeadersDialog(parent, displayLoc, size);
		}
		return dialog;
	}
	...
	/**
	 * Creates new empty Manifest Header and if it not exists in manifest
	 * headers in the bundle, adds the created Manifest Header
	 */
	 ...
	 * @param up
	 *            <code>true</code> if ManifestHeader will be moved one position
	 *            up, <code>false</code> will be moved one position down
	 */
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if (selection != null) {
			...
			if (first instanceof ManifestHeader) {
				...
				if (key.equals(DPPConstants.nameHeader.trim()) || key.equals(DPPConstants.bundleNameHeader.trim()) || key.equals(DPPConstants.bundleVersionHeader.trim()) || key.equals(DPPConstants.dpMissingHeader.trim()) || key.equals(DPPConstants.dpCustomizerHeader.trim())) {
					removeButton.setEnabled(false);
				} else {
					if (isTableEditable) {
	...
	class KeyModifier implements ICellModifier {
		public boolean canModify(Object object, String property) {
			...
			if (key.equals(DPPConstants.nameHeader) || key.equals(DPPConstants.bundleNameHeader) || key.equals(DPPConstants.bundleVersionHeader) || key.equals(DPPConstants.dpMissingHeader) || key.equals(DPPConstants.dpCustomizerHeader)) {
	...
	private void createTable(Composite parent) {
		...
		CellEditor[] editors = new CellEditor[] { new TextCellEditor(table), new TextCellEditor(table) };
	...
	/**
	 * Sets the buttons for moving up and down enable or disable depending on
	...
	public void modelChanged(IModelChangedEvent event) {
		...
		if (changeObject instanceof ManifestHeader) {
			ManifestHeader header = (ManifestHeader) changeObject;
