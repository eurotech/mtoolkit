 ...
 */
public class DPEditorPreferencesPage extends PreferencePage implements IWorkbenchPreferencePage, SelectionListener {
   ...
   * string and if typed value does not exists in the list added in the list of
   * all resource processors.
   */
  ...
  private void editAction() {
    ...
    if (count == 1) {
      ...
      InputDialog dialog = new InputDialog(Display.getCurrent().getActiveShell(), "Edit Resource Processor",
          "Please new ID for selected resource processor", oldValue, new IInputValidator() {
            public String isValid(String newText) {
              if (hasTextInList(newText)) {
                return "Entered resource processor ID is already available";
              } else {
