 ...
 * editors.
 */
public abstract class DPPMultiPageEditor extends EditorPart implements
ShellListener, ISelectionProvider, IFormSelectionListener,
IModelChangedListener {
  ...
  /**
   * The table which holds the page of the editor and the corresponding page
   * id
   */
   ...
   */
  public void removePage(IDPPEditorPage page) {
   ...
   * input object type to determine if it is understood. If not, the
   * implementor must throw a <code>PartInitException</code>.
   * 
   * @param site
   *            the editor site
   * @param input
   ...
   * if this is needed. Examines the editor input object type to determine if
   * it is understood.
   * 
   * @param input
   *            the editor input
  ...
  /**
   * Performs global action in the current editor page for the given
   * identifier.
   * 
   * @param id
   *            the identifier for which will be execute action in current
   *            editor page
   */
  ...
  /**
   * Returns if can paste from clip board. The default return value is
   * <code>true</code>.
   * 
   * @return if in this editor can paste from clip board
   */
  ...
  /**
   * Checks if the given selection can be copied.
   * 
   * @param selection
   *            the selection for which will be checked
   * @return <code>true</code> if selection is not <code>null</code> and is
   *         not empty selection, <code>false</code> otherwise
   */
      ...
      try {
        ...
        if (x.getTargetException().getMessage().endsWith("is read-only.")) {
          ...
          if ((shell != null) && !shell.isDisposed()) {
            String message = ResourceManager.format(RO_ERROR, new String[] { file.getName() });
   ...
   * 
   * @param save
   *            <code>boolean</code> value that signify if the editor changes
   ...
   * new values of the components and sets that this editor is changed and
   * need to be saved.
   * 
   * @param onSave
  ...
  /**
   * Sets the cursor and selection state for this editor to reveal the
  ...
  /**
   * Shows the editor page, which corresponding with the given page
   * identifier.
   * 
   * @param id
   *            the page identifier
