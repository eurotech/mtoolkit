...
public class InstallDialog extends Window implements ConstantsDistributor {
  ...
  private FileDialog getLocationChooser() {
    ...
    String[] names = {(type == INSTALL_BUNDLE_TYPE || type == UPDATE_BUNDLE_TYPE) ? Messages.bundle_filter_label : Messages.deployment_package_filter_label, Messages.all_files_filter_label};
  ...
  public void closeOK() {
	...
	public boolean close() {
		if (!canExit) {
			MessageDialog.openError(getShell(), Messages.standard_error_title,
			                        Messages.file_not_exists);
			canExit = true;
			return false;
	  } 
		return super.close();
