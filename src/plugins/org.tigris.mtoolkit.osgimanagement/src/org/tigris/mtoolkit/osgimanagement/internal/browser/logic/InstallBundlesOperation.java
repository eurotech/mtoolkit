...
import org.tigris.mtoolkit.osgimanagement.installation.FrameworkInstallationItem;
import org.tigris.mtoolkit.osgimanagement.installation.FrameworkProcessor;
import org.tigris.mtoolkit.osgimanagement.internal.FrameWorkView;
import org.tigris.mtoolkit.osgimanagement.internal.Messages;

public class InstallBundlesOperation extends RemoteBundleOperation {
	private java.util.List<FrameworkInstallationItem> items;
	FrameworkProcessor processor;
	private String symbolicName;
	private String version;

	public InstallBundlesOperation(java.util.List<FrameworkInstallationItem> items) {
	...
	protected IStatus doOperation(IProgressMonitor monitor) throws IAgentException {
		...
		for (FrameworkInstallationItem item : items) {
			File bundle = null;
			try {
				bundle = item.getFile();
			} catch (IOException e) {
			...
			try {
				...
				DeviceConnector connector = item.getFramework().getConnector();
				if (connector == null)
				...
				if (!update[0] && !install[0]) {
					...
					bundleIds.addAll(item.getFramework().getBundlesKeys());
				...
				return Util.newStatus(IStatus.ERROR, NLS.bind(Messages.update_file_not_found, bundle.getName()), e);
			} finally {
				if (input != null) {
					try {
						input.close();
			...
			if (FrameworkConnectorFactory.isAutoStartBundlesEnabled && rBundle != null
					&& rBundle[0].getType() != RemoteBundle.BUNDLE_TYPE_FRAGMENT) {
				bundlesToStart.add(rBundle[0]);
			}
		}
		startBundles(bundlesToStart, monitor);
		return Status.OK_STATUS;
	}

	private IStatus startBundles(List<RemoteBundle> bundlesToStart, IProgressMonitor monitor) {
		for (RemoteBundle bundle : bundlesToStart) {
			if (monitor.isCanceled()) {
				return Status.CANCEL_STATUS;
			}
			try {
				startBundleJob(bundle);
	...
	private void startBundleJob(final RemoteBundle remoteBundle) throws IAgentException {
