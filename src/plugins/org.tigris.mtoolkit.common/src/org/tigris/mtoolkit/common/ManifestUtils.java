...
public class ManifestUtils {
	...
	public static Map getManifestHeaders(InputStream stream) throws IOException {
		try {
			...
			IOException ioe = new IOException("JAR Manifest is invalid: "
					+ e.toString());
			ioe.initCause(e);
			throw ioe;
		}
	...
	public static String getBundleSymbolicName(Map headers) {
		try {
			ManifestElement[] element = ManifestElement.parseHeader(
	...
	public static void writeManifest(OutputStream stream, Map headers)
			throws IOException {
	...
	private static void writeEntry(OutputStream out, String key, String value)
			throws IOException {
	...
	private static String splitOnComma(String value) {
		...
		StringBuffer sb = new StringBuffer(value.length()
				+ ((values.length - 1) * LIST_SEPARATOR.length()));
