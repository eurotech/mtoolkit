...
/**
 * Represents remote bundle, installed in the remote OSGi runtime. It enables client to
 * query some of the metadata associated with the bundle, check it state and
 * perform the standard operations - install, uninstall, start, stop and
 * resolve.
 * 
...
public interface RemoteBundle {
   ...
   *             if the underlying bundle is uninstalled
   */
  ...
  /**
   * Returns Manifest headers and corresponding values. If the underlying bundle
   * has been uninstalled its manifest may not be available. In this case, the
   * method will return null.<br>
   * 
   * The header values will be localized according to the passed locale code. If
   * it is null, then the manifest will be returned localized to the current
   * remote locale. If the passed String is empty, then no localization will be
   * performed and the raw values will be returned.
   * 
   * @param locale
   *            the locale code into which the values will be localized
   * @return Dictionary object containing the manifest headers and values of the
   *         underlying bundle
   * @throws IAgentException
   * @throws {@link IllegalStateException}
   *             if the underlying bundle is uninstalled
   ...
   * @throws IAgentException
   * @throws {@link IllegalStateException}
   *             if the underlying bundle has been uninstalled
   */
   ...
   * @throws IAgentException
   * @throws {@link IllegalStateException}
   *             if the underlying bundle has already been uninstalled
   */
   ...
   * 
   * @return true in case of successful resolving, false when the bundle cannot
   *         be resolved
   * @throws IAgentException
   * @throws {@link IllegalStateException}
   *             if the underlying bundle has been uninstalled
   ...
   * @return
   * @throws IAgentException
   * @throws {@link IllegalStateException} if the bundle is uninstalled
   */
   ...
   * </ul>
   * 
   * If none of the above values is valid for this bundle, then result 0 (zero)
   * will be returned
   * 
   * @return
   * @throws IAgentException
   ...
   * bundle or null if the bundle has no attached fragments or it isn't fragment
   * 
   * @return
   * @throws IAgentException
   * @throws {@link IllegalStateException} if the bundle is uninstalled
   ...
   * fragment is attached or null if the bundle is not a fragment. Currently, a
   * fragment can be attached to only one host.
   * 
   * @return
   * @throws IAgentException
   * @throws {@link IllegalStateException} if the bundle is uninstalled
   ...
   * system bundles are the bundles which are part of the platform and shouldn't
   * be managed by users.
   * 
