...
public class ConnectionManagerImpl implements ConnectionManager {
  ...
  public AbstractConnection createConnection(int type) throws IAgentException {
    ...
    // Connection creation is done in two steps. This have the side effect that DISCONNECT event
    // can be delivered after CONNECT event.
    
    // Step 1: check whether there is a stale connection is closed/closing
    // If there is a closed/closing connection, save the reference and clear the active connection reference
    // This gives us two things: 1. event won't be fired from the connectionClosed() method
    ...
    // If stale connection is detected, try to close it (just in case) and fire an event for the disconnection
    if (staleConnection != null) {
      staleConnection.closeConnection();
      fireConnectionEvent(ConnectionEvent.DISCONNECTED, staleConnection);
    }
    // Step 2: check whether there is an active connection. Create if necessary.
    // It is very unlikely to get stale connection here, because we have already checked it in Step 1
    synchronized (this) {
      switch (type) {
      case PMP_CONNECTION:
  ...
  public void closeConnections() throws IAgentException {
    ...
    // only call closeConnection() because it will result in connectionClosed()
