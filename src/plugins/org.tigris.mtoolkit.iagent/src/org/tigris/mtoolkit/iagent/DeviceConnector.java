...
/**
 * This class represents connection to a remote OSGi framework. It is associated to real
 * connection over some transport API. It provides the command groups with
 * available commands to the connected OSGi framework.
 * 
 * @version 1.0
 */
public abstract class DeviceConnector {
  ...
  /**
   * Provides DeviceConnector connected to specified remote OSGi framework over client
   * connection.
   * 
   ...
   *            contain the host property. They must be mapped with specified
   *            keys - {@link DeviceConnector#KEY_DEVICE_IP}. The value for the
   *            host must be String.
   * @return DeviceConnector object which is connected to the specified remote OSGi framework
  ...
  public final static DeviceConnector openClientConnection(int aConnetionType, Dictionary aConProps)
    ...
    throw new IAgentException("No available factory for connection type: " + aConnetionType, IAgentErrors.ERROR_INTERNAL_ERROR);
  }

  /**
   * When called this method opens server connection with the specified type and
   * properties and blocks until client connection from remote OSGi framework is accepted.
   * After connection acceptance it returns DeviceConnector associated to the
   ...
   *            timeout, etc.). At least these properties must contain the port
   *            number. It must be inserted with the specified key
   *            {@link DeviceConnector#KEY_PORT} and the value must be an
   *            Integer object. If passed the timeout property must be mapped to
  ...
  /**
   * Closes connection to the remote OSGi framework which this {@link DeviceConnector}
   * provides. After calling this method the provided command utilities will
   ...
   * {@link DeviceConnectionListener#disconnected(DeviceConnector)} method will be called when
   * the connection this {@link DeviceConnector} is working over is
   * disconnected.
   * 
   * @param aListener
