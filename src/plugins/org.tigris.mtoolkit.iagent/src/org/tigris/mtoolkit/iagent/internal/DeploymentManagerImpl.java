...
public class DeploymentManagerImpl implements DeploymentManager, EventListener, ConnectionListener {
  ...
  public RemoteBundle[] getBundles(String symbolicName, String version) throws IAgentException {
    ...
    long[] bids = (long[]) Utils.callRemoteMethod(getBundleAdmin(), Utils.GET_BUNDLES_METHOD, new Object[] { symbolicName, version});
    if (bids == null) {
  ...
  public RemoteDP getDeploymentPackage(String name) throws IAgentException {
    ...
    if (name == null)
      throw new IllegalArgumentException();
    String version = (String) Utils.callRemoteMethod(getDeploymentAdmin(), Utils.GET_DP_VERSION_METHOD, new Object[] {name});
    if (version == null) {
  ...
  public RemoteBundle installBundle(String location, InputStream is) throws IAgentException {
    ...
    if (location == null || is == null)
      throw new IllegalArgumentException();
    Long bundleId = (Long) Utils.callRemoteMethod(getBundleAdmin(), Utils.GET_BUNDLE_BY_LOCATION_METHOD, new Object[] {location});
    if (bundleId != null) {
      if (bundleId.longValue() != -1)
        return new RemoteBundleImpl(this, bundleId, location);  // directly return reference to the bundle, instead of walking to the device for this
    }
    Object result = Utils.callRemoteMethod(getBundleAdmin(), Utils.INSTALL_BUNDLE_METHOD, new Object[] {location, is});
  ...
  public RemoteBundle[] listBundles() throws IAgentException {
    ...
    if (bids == null) {
      ...
      throw new IAgentException("listBundles() must not return null array. There is a problem with the transport.", IAgentErrors.GENERAL_ERROR);
    }
    RemoteBundle[] bundles = new RemoteBundle[bids.length];
  ...
  public RemoteDP[] listDeploymentPackages() throws IAgentException {
    ...
    Dictionary dps = (Dictionary) Utils.callRemoteMethod(getDeploymentAdmin(), Utils.LIST_DPS_METHOD, new Object[] {});
  ...
  private void fireDeploymentEvent(String symbolicName, String version, int type) {
    ...
    for (int i = 0; i < listeners.length; i++) {
      ...
      try {
        ...
        IAgentLog.error("[DeploymentManagerImpl][fireDeploymentEvent] Failed to deliver event to " + listener, e);
      }
    }
  }
  ...
  public void event(Object event, String eventType) {
    try {
     ...
     if (DEPLOYMENT_EVENT.equals(eventType)) {
      ...
      IAgentLog.error("[DeploymentManagerImpl][event] Failed to process PMP event: " + event + "; type: " + eventType);
    }
  }
  ...
  public void connectionChanged(ConnectionEvent event) {
    ...
    if (event.getType() == ConnectionEvent.CONNECTED && event.getConnection().getType() == ConnectionManager.PMP_CONNECTION) {
      ...
      systemBundlesIDs = null;  // reset system bundle ids - we don't know whether it was VM restart or PMP connection closed
        ...
        synchronized (bundleListeners) {
	          ...
	          if (dpListeners != null && dpListeners.size() > 0) {
	            ...
	            try {
	              ...
	              IAgentLog.error("[DeploymentManagerImpl][connectionChanged] Failed to add event listener to PMP connection", e);
	            }
	          }
  ...
  private long[] getSystemBundlesIDs() throws IAgentException {
    ...
    long[] result = (long[]) Utils.callRemoteMethod(getBundleAdmin(), Utils.GET_SYSTEM_BUNDLES_IDS_METHOD, new Object[0]);
    if (result == null) {
      log("[getSystemBundlesIDs] getSystemBundlesIDs() must not return null array. There is a problem with the transport.");
      throw new IAgentException("getSystemBundlesIDs() must not return null array. There is a problem with the transport.", IAgentErrors.ERROR_INTERNAL_ERROR);
    }
    PMPConnection connection = getConnection(false);
    if (connection != null) {
