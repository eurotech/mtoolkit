...
/**
 * Responsible for managing deployment units inside the OSGi
 * Runtime. It supports bundles and deployment packages. <br>
 * 
...
public interface DeploymentManager {
   ...
   *             if the passed symbolicName is null
   */
   ...
   * the install is successful, a RemoteDP object is returned which can be used
   * to manage and introspect the installed DP.<br>
   * 
   * If the installation fails, {@link IAgentException} will be thrown. Its code
   * will be equal to the corresponding DeploymentException code.
   * 
   * @param is
   *            InputStream with the deployment package's contents
   * @return RemoteDP object representing the newly installed/updated deployment
   *         package
   * @throws IAgentException
   */
  ...
  /**
   * Returns all installed bundles currently available in the runtime. The returned array contains
   * RemoteBundle objects, which can be used to query various information and to manage the underlying bundle. 
   *  
  ...
  /**
   * Returns bundles with given symbolic name and version. If the version is null, then all bundles
   * with this symbolic name are returned. If no bundle exists with this symbolic name, null is returned.
   * 
   * @param symbolicName the symbolic name of the bundle to return. If null is passed, {@link IllegalArgumentException} is thrown.
   * @param version the version of the bundle. In case of null, all bundles with given symbolic name are returned
   * @return array of RemoteBundle objects. When there are no bundles to return, null array is returned.
   * @throws IAgentException
   * @throws {@link IllegalArgumentException}
   */
  ...
  /**
   * Installs the bundle from the passed {@link InputStream} object and returns RemoteBundle
   * object representing the newly installed bundle. In case of failure, IAgentException is thrown.
   * 
   * @param location the location to be assigned to the newly installed bundle
   * @param is InputStream object containing the bundle
  ...
  /**
   * Add a listener which will be notified whenever bundle event is generated on
   * the remote site. Adding the same listener twice doesn't have any effect
   * 
   * @param listener
  ...
  /**
   * Add a listener to be notified when deployment package event is generated on
   * the remote site. Adding the same listener twice doesn't have any effect.
   * 
   * @param listener
